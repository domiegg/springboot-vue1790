<template>
	<div class="diy_details_box diy_div_scheduling_scheme">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "scheduling_scheme_id",
					source_id: this.obj["scheduling_scheme_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/scheduling_scheme/set?scheduling_scheme_id=' + _this.obj["scheduling_scheme_id"], {
								praise_len
							}, (res) => {
								if(res.result){
