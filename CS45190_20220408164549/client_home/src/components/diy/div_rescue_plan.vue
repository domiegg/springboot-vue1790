<template>
	<div class="diy_details_box diy_div_rescue_plan">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "rescue_plan_id",
					source_id: this.obj["rescue_plan_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/rescue_plan/set?rescue_plan_id=' + _this.obj["rescue_plan_id"], {
								praise_len
							}, (res) => {
								if(res.result){
