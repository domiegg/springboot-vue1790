<template>
	<div class="diy_details_box diy_div_material_distribution">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "material_distribution_id",
					source_id: this.obj["material_distribution_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/material_distribution/set?material_distribution_id=' + _this.obj["material_distribution_id"], {
								praise_len
							}, (res) => {
								if(res.result){
