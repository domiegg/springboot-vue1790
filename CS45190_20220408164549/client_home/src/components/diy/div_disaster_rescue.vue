<template>
	<div class="diy_details_box diy_div_disaster_rescue">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "disaster_rescue",
					source_field: "disaster_rescue_id",
					source_id: this.obj["disaster_rescue_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/disaster_rescue/set?disaster_rescue_id=' + _this.obj["disaster_rescue_id"], {
								praise_len
							}, (res) => {
								if(res.result){
