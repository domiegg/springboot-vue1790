<template>
	<div class="diy_details_box diy_div_material_inventory">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "material_inventory_id",
					source_id: this.obj["material_inventory_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/material_inventory/set?material_inventory_id=' + _this.obj["material_inventory_id"], {
								praise_len
							}, (res) => {
								if(res.result){
