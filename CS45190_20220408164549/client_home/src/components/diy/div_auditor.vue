<template>
	<div class="diy_details_box diy_div_auditor">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "auditor_id",
					source_id: this.obj["auditor_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/auditor/set?auditor_id=' + _this.obj["auditor_id"], {
								praise_len
							}, (res) => {
								if(res.result){
