<template>
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','personnel_number') || $check_field('add','personnel_number') || $check_field('set','personnel_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="人员编号" prop="personnel_number">
					<el-input id="personnel_number" v-model="form_sub['personnel_number']" placeholder="请输入人员编号"
							  v-if="user_group === '管理员' || (form_sub['auditor_id'] && $check_field('set','personnel_number')) || (!form_sub['auditor_id'] && $check_field('add','personnel_number'))"></el-input>
					<div v-else-if="$check_field('get','personnel_number')">{{form_sub['personnel_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','personnel_name') || $check_field('add','personnel_name') || $check_field('set','personnel_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="人员姓名" prop="personnel_name">
					<el-input id="personnel_name" v-model="form_sub['personnel_name']" placeholder="请输入人员姓名"
							  v-if="user_group === '管理员' || (form_sub['auditor_id'] && $check_field('set','personnel_name')) || (!form_sub['auditor_id'] && $check_field('add','personnel_name'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','personnel_gender') || $check_field('add','personnel_gender') || $check_field('set','personnel_gender')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="人员性别" prop="personnel_gender">
					<el-select id="personnel_gender" v-model="form_sub['personnel_gender']"
						v-if="user_group === '管理员' || (form_sub['auditor_id'] && $check_field('set','personnel_gender')) || (!form_sub['auditor_id'] && $check_field('add','personnel_gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','personnel_age') || $check_field('add','personnel_age') || $check_field('set','personnel_age')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="人员年龄" prop="personnel_age">
					<el-input id="personnel_age" v-model="form_sub['personnel_age']" placeholder="请输入人员年龄"
							  v-if="user_group === '管理员' || (form_sub['auditor_id'] && $check_field('set','personnel_age')) || (!form_sub['auditor_id'] && $check_field('add','personnel_age'))"></el-input>
					<div v-else-if="$check_field('get','personnel_age')">{{form_sub['personnel_age']}}</div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["auditor_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?auditor_id=" + form_sub["auditor_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/auditor/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/auditor/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/auditor/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/auditor/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/auditor/view','get');
